...
'''Function to save the weights of the trained model as json file.
The weights are the same as model parameters, w_0, w, and V.'''
...
def save_model_weights(model, filename):
    ...
    if hasattr(model, 'global_bias'):
        model_data["global_bias (w_0)"] = float(model.global_bias)
    if hasattr(model, 'weights'):
        model_data["weights (w)"] = model.weights.tolist() if isinstance(model.weights, np.ndarray) else model.weights
    if hasattr(model, 'pairwise_interactions'):
        model_data["pairwise_interactions (V)"] = model.pairwise_interactions.tolist() if isinstance(model.pairwise_interactions, np.ndarray) else model.pairwise_interactions
...
if __name__ == "__main__":
    ...
    weights_filename = f"Results/weights_{file_base_name}"
    ...
    for ds in dataset:
        ...
        for train_index, test_index in cv_split.split(Y):
