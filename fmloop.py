...
if __name__ == "__main__":
    ...
    # List to store final C-index values for each dataset and feature option
    final_cindex_results = []
    ...
    print("\nRunning the DTI prediction model...\n")
    
    # Track progress
    total_iterations = len(datasets) * len(feature_options) * num_cv_folds_split
    progress_bar = tqdm(total = total_iterations, leave = False)
    ...
    for ds in datasets:
        ...
        for feature_option in feature_options:
            ...
            final_cindex_results.append({
                "Dataset": ds,
                "Feature option": feature_option,
                "C-index": np.mean(fold_results)
            })

    print("\n\nC-indexes across all used datasets and feature options:\n")
    for result in final_cindex_results:
        print(result)
    print("")