...
This repository contains the implementation for my Master of Science (Technology) Thesis in Biomedical Engineering at the University of Turku. The project focuses on developing a machine learning (ML) pipeline for predicting drug–target interactions (DTI) using factorisation machine (FM). Brifely, I developed a predictive ML model using multimodal drug–target data and factorisation machine. Achieved C ≥ 0.83 across all 7 datasets. The thesis is available to read in [UTUPub](https://www.utupub.fi/handle/10024/181501).
